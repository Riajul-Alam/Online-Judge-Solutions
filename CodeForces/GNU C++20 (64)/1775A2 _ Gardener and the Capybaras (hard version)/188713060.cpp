...
void solve()
{
    ll m,c1=0;string tm1;
    vector<string>start,End;
    string st;
    cin>>st;m=st.size();
    deque<char>dq,dq1,tm2;
    for(int i=2;i<m;i++){
      dq.pb(st[i]);
    }
    tm2.pb(st[0]);
    for(int i=1;i<m-1;i++){
      deque<char>tmp;tmp.pb(st[i]);
      if(tm2<=tmp && dq<=tmp){
        for(int j=0;j<sz(tm2);j++)cout<<tm2[j];
        cout<<" ";
        for(int j=0;j<sz(tmp);j++)cout<<tmp[j];
        cout<<" ";
        for(int j=0;j<sz(dq);j++)cout<<dq[j];
        cout<<endl;return;
      }
      else if(tm2>=tmp && dq>=tmp){
        for(int j=0;j<sz(tm2);j++)cout<<tm2[j];
        cout<<" ";
        for(int j=0;j<sz(tmp);j++)cout<<tmp[j];
        cout<<" ";
