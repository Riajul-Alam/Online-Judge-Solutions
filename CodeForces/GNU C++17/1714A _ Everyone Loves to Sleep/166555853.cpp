...
void solve()
{
    ll h,n,m,i,j,k,x,y,z,sum=0,c1=0,c2=0,ans=0;
    cin>>n>>h>>m;
    vector<ll>a(n);
    ans=0;
    ll Ttime=0,Thour=0;
    map<pll,ll>mp;
    vector<pll>vp,answer;
    for(int i=0;i<n;i++)
    {
      cin>>x>>y;vp.pb({x,y});
      mp[{x,y}]++;
    ...
    {
      if(h==vp[i].ff && m==vp[i].ss)continue;
      ll hour=0,time=0;
      if(h>vp[i].ff || (h>=vp[i].ff && m>vp[i].ss)){
       hour=24-h;
       if(m>0){
        time=60-m;hour--;
       }
    ...
    sort(all(answer));
    cout<<answer[0].ff<<" "<<answer[0].ss<<endl;
