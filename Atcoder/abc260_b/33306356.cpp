...
bool cmp(const pair<ll,pair<ll,ll>>a, const pair<ll,pair<ll,ll>>b)
{
    if(a.first == b.first)
    {
        return (a.second.ff < b.second.ff);
    }
...
void solve()
{
    ...
    f0(n)cin>>b[i];
    vector<pair<ll,pair<ll,ll>>>vp,vp1,vp2;
    for(int i=0;i<n;i++){
    	vp.pb({a[i],{i+1,b[i]}});
    }
    sort(all(vp),cmp);
    ...
    f0(n){
    	c1++;
    	if(c1<=x){an.pb(vp[i].ss.ff);}
    	else{
    		vp1.pb({vp[i].ss.ss,{vp[i].ss.ff,vp[i].ff}});
    	}
    }
    sort(all(vp1),cmp);debug(vp1)
    vector<pll>vv;
    c1=0;
    for(int i=0;i<sz(vp1);i++){
    	c1++;
    	if(c1<=y){
    		an.pb(vp1[i].ss.ff);
    	}else{
    		vv.pb({vp1[i].ff+vp1[i].ss.ss,vp1[i].ss.ff});
    	}
    }
    sort(all(vv),cmp1);
    c1=0;
