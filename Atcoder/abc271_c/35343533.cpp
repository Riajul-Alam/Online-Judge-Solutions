...
class Solution {
public:
    int lengthOfLIS(vector<pll>& nums) {
        vector<int>ans;
        ans.pb(nums[0].ss);
        for(int i=1;i<nums.size();i++)
        {
          if(nums[i].ss>ans.back())ans.push_back(nums[i].ss);
          else{
            int idx=LB(ans,nums[i].ss);
            ans[idx]=nums[i].ss;
          }
        }
        return sz(ans);
...
void solve()
{
    ...
    for(int i=0;i<n;i++)
    {
      cin>>a[i].ff;b[i].ss=a[i].ff;
    }
    f0(n){
      cin>>a[i].ss;b[i].ff=a[i].ss;
    }
    sort(all(a));
    Solution ob,ob1;
    ans=n+ob.lengthOfLIS(a);
    sort(all(b));
    ans=max(ans,ob1.lengthOfLIS(b)+n);
    cout<<ans<<endl;
