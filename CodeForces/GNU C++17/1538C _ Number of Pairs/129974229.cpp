...
{   
 ...
 while(t--)
 {
   ...
   for(int i=1;i<n;i++)
   {
    ll lo=os.order_of_key({x-a[i],-1});
    ll hi=os.order_of_key({y-a[i],INF});
    ans+=(hi-lo);os.insert({a[i],i+1});
   }
   cout<<ans<<endl;
 }
}
void solve()
{
 ...
 while(t--)
 {
   ...
   for(int i=1;i<n;i++)
   {
    ll lo=os.order_of_key(x-a[i]);
    ll hi=os.order_of_key((y-a[i])+1);
    ans+=(hi-lo);os.insert(a[i]);
   }
   cout<<ans<<endl;
 }
}
