...
void solve()
{
    ...
    for(auto q:mp){
     c2++;
     ll cur=-1,prev=-1;c1=0;
     x=q.ff;
     sort(rall(mp[x]));//debug(mp[x])
     ll v1,v2,v3;
     if(sz(mp[x])==2){
      res.pb({mp[x][0].ss,mp[x][1].ss});
     }
     else if(sz(mp[x])>=3){
      //bug2(mp[x][1].ff,mp[x][2].ff)
      if(mp[x][1].ff==mp[x][2].ff){
       res.pb({"",""});
      }else{
       res.pb({mp[x][0].ss,mp[x][1].ss});
      }
    ...
    for(int i=0;i<sz(res);i++){
     if(res[i].ff=="")cout<<"?"<<endl;
     else cout<<res[i].ff<<" "<<res[i].ss<<endl;
    }
}
