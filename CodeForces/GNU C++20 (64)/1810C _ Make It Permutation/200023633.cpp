...
void solve()
{
    ll n,m,c,d,k,x,y,z,sum=0,c1=0,c2=0,ans=INF;
    cin>>n>>c>>d;
    vector<ll>a(n);
    map<ll,ll>mp;
    for(int i=0;i<n;i++)
    ...
    for(auto q:mp){
      c1=(q.ss-1);
      if(x==q.ff){
        x++;ans2+=(c*c1);Count+=(c1+1);
      }else{
        ll len=(q.ff-x);debug(len)
        ll rl=(n-Count);bug2(Count,rl)
        ans=min(ans,(rl*c)+ans2);
        z=(len*d);x=q.ff+1;Count+=(c1+1);
        ans2+=(z+(c1*c));
      }
    }
    ans=min(ans,ans2);
    if(ans==INF)ans=(n-Count)*c;
    cout<<ans+add<<endl;
