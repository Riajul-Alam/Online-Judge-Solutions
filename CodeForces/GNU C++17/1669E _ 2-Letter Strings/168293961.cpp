...
void solve()
{
    ...
    for(int i=0;i<n;i++)
    {
     ll totalCount=m1[a[i]];
     ll FirstCount=First[a[i][0]];
     ll SecondCount=Second[a[i][1]];
     ll cur=0;
  cur=(FirstCount-totalCount);
  ans+=(cur);
  cur=0;
  cur=(SecondCount- totalCount);
  ans+=(cur);
     bug2(FirstCount,SecondCount)
    }
    cout<<ans/2<<endl;
