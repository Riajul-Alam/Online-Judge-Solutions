...
void solve()
{
    ...
    ll st,end,last=final[0].ff;x=0;
    st=final[0].ff;
    end=final[0].ff;
    c1=1;
    for(int i=1;i<sz(final);i++)
    {
     if(final[i].ff!=(last+1)){
      vp.pb({c1,{st,end}});
      st=end=final[i].ff;c1=1;last=final[i].ff;
     }else{
      c1++;
      end=final[i].ff;
      last=final[i].ff;
    ...
    debug(vp)
    sort(rall(vp));
    cout<<vp[0].ss.ff<<" "<<vp[0].ss.ss<<endl;
...
/*    c1=0;
    ans=0;
    vector<pair<ll,pair<ll,ll>>>vp;
    ll st,end,last=1;x=0;
    for(auto q:mp){
     if(q.ss>=k && (last==q.ff || c1==0)){
      if(c1==0){
       st=end=q.ff;
      }else{
       end=q.ff;
 ...
 debug(vp)
 sort(rall(vp));//debug(vp)
 if(sz(vp)==0){
   cout<<-1<<endl;return;
 }
    cout<<vp[0].ss.ff<<sp<<vp[0].ss.ss<<endl;*/
