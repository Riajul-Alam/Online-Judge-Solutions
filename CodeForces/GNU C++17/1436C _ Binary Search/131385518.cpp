...
{   
 ...
 if(l>0 && (l-1)==k){
  ll leftPossible=0;
  if(m-1>=0 && m-L-1>=0)leftPossible=modMul(fact[m-1],inverse[m-L-1]);
  ll rightPossible=0;
  if(n-m>=0 && n-m-G>=0)rightPossible=modMul(fact[n-m],inverse[n-m-G]);
  ll disPossible=fact[n-G-L-1];
  ll totalPossible=(disPossible%mod);
