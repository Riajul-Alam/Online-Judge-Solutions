...
void solve()
{
    ll n,m,w,h,k,x,y,z,sum=0,c1=0,c2=0,ans=0;
    cin>>n>>w>>h;
    vector<pll>cake,choc;
    for(int i=0;i<n;i++){
      cin>>x;
      cake.pb({x-w,x+w});
    }
    ...
    // fit 
    for(int i=0;i<n;i++){
      if(choc[i].ff>=cake[i].ff && choc[i].ss<=cake[i].ss){
        c1++;
      }
    }
    if(c1==n){
    ...
    if(c1==n){
      for(int i=0;i<n;i++){
        x=choc[i].ff-cake[i].ff;
        y=choc[i].ss-cake[i].ss;
        mx=max(mx,y);
        mn=min(mn,x);
      }
      if(mn>=mx){
    ...
    if(c2==n){
      for(int i=0;i<n;i++){
        x=cake[i].ff-choc[i].ff;
        y=cake[i].ss-choc[i].ss;
        mx=max(mx,x);
        mn=min(mn,y);
      }
      if(mn>=mx){
