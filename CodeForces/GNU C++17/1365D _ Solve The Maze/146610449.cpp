...
void solve()
{
  ...
  if(a[i][j]=='B'){
   ...
   if(i-1>=0)g[i-1][j]=1;
   if(j-1>=0)g[i][j-1]=1;
   //check any Adjacent Good,Bad 
   if(j+1<m && a[i][j+1]=='G')found=1;
   if(i+1<n && a[i+1][j]=='G')found=1;
   if(i-1>=0 && a[i-1][j]=='G')found=1;
   if(j-1>=0 && a[i][j-1]=='G')found=1;
  }
     }
    }
