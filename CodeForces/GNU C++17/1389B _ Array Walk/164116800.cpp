...
ll dp[N][10],n,k,z;
ll a[N];
ll f(ll i,ll move,ll leftOnly)
{
 if(move==k)return 0;
 if(dp[i][leftOnly]!=-1){
  return dp[i][leftOnly];
 }
 ll x=0,y=0;
 if(i+1<n && move+1<=k){
  x=a[i+1]+f(i+1,move+1,leftOnly);
 }
 if(i-1>=0 && move+1<=k && leftOnly+1<=z){
  y=a[i-1]+f(i-1,move+1,leftOnly+1);
 }
 dp[i][leftOnly]=max(x,y);
 return dp[i][leftOnly];
}
 
void solve()
{
    ll m,i,j,x,y,sum=0,c1=0,c2=0,ans=0;
    cin>>n>>k>>z;
    f0(n){
     f00(10){
      dp[i][j]=-1;
