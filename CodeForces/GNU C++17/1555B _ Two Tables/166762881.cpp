...
void solve()
{
    ll n,m,h1,w1,x1,y1,x2,y2,h,w,i,j,k,x,y,z,sum=0,c1=0,c2=0,ans=0;
    cin>>w>>h;
    cin>>x1>>y1>>x2>>y2;
    cin>>w1>>h1;
    ll ansh=INF,answ=INF;
    ll hdif=0,wdif=0,h2,w2;
    w2=(x2-x1),h2=(y2-y1);
    hdif=(h-h1);wdif=(w-w1);
    if(h2+h1<=h || w1+w2<=w)
    {
     if(h1+h2<=h)
     {
      ansh=min((h1-y1),h1-(h-y2));
      if(ansh<0)ansh=0;
     }
     if(w1+w2<=w)
     {
      answ=min((w1-x1),w1-(w-x2));
      if(answ<0)answ=0;
     }
    }else ansh=-1;
 
    double anss=min(ansh,answ);
