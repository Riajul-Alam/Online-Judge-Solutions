...
bool good(vector<ll>&a,ll mid,ll coin,ll y)
{
 ll n=sz(a);
 ll sum=0,c1=0;ll total=0;ll xx=0;
 while(sum<coin){
  ll day=0,c2=0;
  for(int i=0;i<n;i++){
   sum+=a[i];day++;c2++;
   if(sum>=coin && total+day<=y){
    return 1;
   }
   if(day>mid){
    break;
  ...
  if(day>mid)total+=day;
  else total+=(day+(1+mid-day));
  if(total>y)return 0;
 }
 return total<=y;
}
...
void solve()
{
    ll n,m,k,x,y,z=0,sum=0,c1=0,c2=0,ans=0;
    cin>>n>>x>>y;
    vector<ll>a(n);
    for(int i=0;i<n;i++)
    {
