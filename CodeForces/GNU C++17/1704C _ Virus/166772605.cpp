...
void solve()
{
    ...
    for(int i=0;i<m;i++)
    {
     if(i==0){
      an.pb(a[i]-1);
      if(m!=1)an.pb(a[i+1]-a[i]-1);
      else an.pb(max(n-1,ll(0)));
     }
     else if(i==m-1){
      an.pb(n-a[i]);
     }
    ...
    debug(an)
    c1+=m;
    if(sz(an)>0)an[sz(an)-1]+=an[0];
    if(sz(an)>0)an.erase(an.begin());
    sort(rall(an));
    debug(an)
    ll infected=0;
    ...
    {
     if(an[i]-infected>2){
      ans+=(an[i]-infected-1);infected+=4;
     }
     else if(an[i]-infected==1 ||an[i]-infected==2){
      ans++;infected+=2;
     }
     else if(an[i]-infected<=0)continue;
    }
    debug(ans)
    cout<<n-ans<<endl;
