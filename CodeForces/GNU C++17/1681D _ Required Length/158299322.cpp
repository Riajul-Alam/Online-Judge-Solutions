...
void solve()
{
    ll n,x,m,y,z,k,sum=0,c1=0,c2=0,ans=INF;
    cin>>n>>x;
    queue<ll>q;
    q.push(x);
    ll Count=0;
    ...
    while(!q.empty())
    {
     ...
     ll v=u;
     string st=tostr(v);
     if(sz(st)==n){ans=min(ans,level[v]);continue;}
     else if(sz(st)>n)continue;
     for(int i=0;i<sz(st);i++)
     {
      if(st[i]=='0' || st[i]=='1'){continue;}
      ll tm=st[i]-'0';
      if(mp[u*tm]==0){q.push(u*tm);mp[u*tm]++;level[u*tm]=level[u]+1;}
     }
    }
    cout<<((ans==INF)? -1:ans);
}
