...
void solve()
{
    ...
    for(ll i=0;i<n;i++)
    {
     if(!vis[a[i]]){
      ...
      if(sz(mp[a[i]])>1){
       ll p=0,q=1;
       ll mn=mp[a[i]][q]-mp[a[i]][p];
       for(ll j=1;j<sz(mp[a[i]]);j++)
       {
        if(mn>(mp[a[i]][j]-mp[a[i]][j-1])){
         p=j-1,q=j;mn=(mp[a[i]][j]-mp[a[i]][j-1]);
        }
       }
       c1++;
       x=0,y=0;
