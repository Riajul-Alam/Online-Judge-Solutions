...
ll f(ll i,ll j,ll remain)
{
 ...
 ll ans=INF;
 if(i>0){
  ans=min(ans,vertical[i-1][j]+f(i-1,j,remain-1));
 }
 if(i+1<n){
  ans=min(ans,vertical[i][j]+f(i+1,j,remain-1));
 }
 if(j>0){
  ans=min(ans,horizontal[i][j-1]+f(i,j-1,remain-1));
 }
 if(j+1<m){
  ans=min(ans,horizontal[i][j]+f(i,j+1,remain-1));
 }
 dp[i][j][remain]=ans;
 return dp[i][j][remain];
}
