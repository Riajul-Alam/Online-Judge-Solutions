...
void solve()
{
    ...
    {
      cin>>a[i].ff>>a[i].ss;
      if((a[mxIndex].ff<a[i].ff)|| (a[i].ff==a[mxIndex].ff && a[i].ss<a[mxIndex].ss)){
      	mxIndex=i;
      }
    }
    vector<ll>visit(n+5,0);
    for(int i=0;i<n;i++){
    	ll currentMinIndex=mxIndex;
    	for(int j=0;j<n;j++){
    		if(visit[j]==1)continue;
    		if(a[currentMinIndex].ff==a[j].ff){
    			if(a[currentMinIndex].ss<=a[j].ss){
    				currentMinIndex=j;
    			}
    		}else if(a[currentMinIndex].ff>=a[j].ff){
    			currentMinIndex=j;
    		}
    	}
    	visit[currentMinIndex]=1;
    	cout<<currentMinIndex+1<<endl;
    }
}
