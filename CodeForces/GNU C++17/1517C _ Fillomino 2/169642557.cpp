...
#define eps 1e-10
#define INF 1e18
bool valid(ll x,ll y,ll nx,ll my,vector<vector<ll>>&visited){
 return (x>=0 && x<nx && y>=0 && y<my && visited[x][y]==-1);
}
void solve()
{
    ...
    for(ll i=0;i<n;i++)
    {
     ...
     a[x][y]=a1[i];
     while(cur>c1){
      if(valid(x,y-1,n,n,a)){
       a[x][y-1]=cur;c1++;y--;
      }
      else if(valid(x+1,y,n,n,a)){
       a[x+1][y]=cur;c1++;x++;
      }
      else if(valid(x,y+1,i,i,a)){
       a[x][y+1]=cur;c1++;y++;
      }
     }
    }
