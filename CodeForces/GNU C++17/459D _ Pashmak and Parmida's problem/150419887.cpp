...
void solve()
{
    ...
    for(int i=n-1;i>=0;i--){
     mp1[a[i]]++;
     right[i]=mp1[a[i]];os.insert({right[i],i});
    }
    debug(left)
    debug(right)
    //final=right;
    ...
    for(int i=0;i<n;i++)
    {
     /*auto tm = find(all(right),final[i]);
        ll index=tm-right.begin();
        if(index!=right.size())right.erase(right.begin()+index);
        debug(right)
     auto q=LB(right,left[i]);
     debug(q);
     if(q!=0){
      ans+=q;
     }*/
     debug(right[i]);debug(i);
     os.erase({right[i],i});
     x=os.order_of_key({left[i],i});
     ans+=x;
     debug(os)
    }
    cout<<ans<<endl;
