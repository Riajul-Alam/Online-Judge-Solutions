...
void solve()
{
    ...
    for(auto q1:m3){
      if(q1.ss>2){
        ...
        p[mp[q1.ff][0]]=q1.ff;m1[q1.ff]++;
      }
      else if(q1.ss==2){
        p[mp[q1.ff][0]]=q1.ff;m1[q1.ff]++;
        vp2.pb({q1.ff,mp[q1.ff][1]});
        q[mp[q1.ff][1]]=q1.ff;m2[q1.ff]++;
      }
    }
    vector<ll>avail,avail1;
    for(int i=1;i<=n;i++){
      if(m1[i]==0){
        avail.pb(i);
      }
...
/*    for(int i=0;i<n;i++){
    ...
    for(int i=0;i<sz(vp2);i++){
      p[vp2[i].ss]=avail[j];j++;
    }
    for(int i=0;i<n;i++){
      vp1.pb({p[i],i});
    }
    ...
    for(int i=0;i<n;i++){
      if(q[vp1[i].ss]==0){
        q[vp1[i].ss]=avail1[j];j++;
      }
    }
    set<int>s1,s2;vector<ll>final(n);
    f0(n){
      s1.in(p[i]);
      s2.in(q[i]);
      final[i]=max(p[i],q[i]);
    }
    if(sz(s1)==n && sz(s2)==n && a==final){
      cout<<"YES"<<endl;
      print(p);
      print(q);
    }
