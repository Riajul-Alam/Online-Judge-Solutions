...
void solve()
{
    ll n,m,k,x,y,c3=0,z,sum=0,c1=0,c2=0,ans=0;
    cin>>n;
    vector<ll>a(n),b,c,d;
    set<ll>s;
    for(int i=0;i<n;i++)
    {
      cin>>a[i];s.in(a[i]);
      if(a[i]==0)c3++;
      if(a[i]>0){c1++;b.pb(a[i]);}
      if(a[i]<0){c2++;c.pb(a[i]);}
    }
    if(c3==n){
       cout<<"No"<<endl;return;
    }
    sort(all(c));
    sort(rall(b));
    ll i=0,j=0;ll sum2=0;
    ll dif=(mxv(a)-mnv(a));
    while(i<sz(b) && j<sz(c) && c1<n){
      sum=0;c1++;
      while(sum2+b[i]<dif && i<sz(b)){
        d.pb(b[i]);
        sum2+=b[i];i++;
      }
      while(sum2+b[i]>=dif && j<sz(c)){
        d.pb(c[j]);
        sum2+=c[j];j++;
      }
    }
