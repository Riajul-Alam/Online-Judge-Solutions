...
void solve()
{
    ...
    if(t1==t2){
      ...
      s2+=s1[sz(s1)-1];final=s2;
    }
    debug(final)
    if(palindrome(final,sz(final))){
      cout<<"YES"<<endl;
    }else{
       cout<<"NO"<<endl;
    }
...
/*    b=a;
    string last;
    x=1;c1=0;
    vector<bool>visit(m+5,0);
    for(int i=0;i<n;i++){
      for(int j=0;j<m;j++){
        if(sz(a[j])==x){
          y=0;
          for(int k1=0;k1<sz(last);k1++){
            if(last[k1]!=a[j][k1]){
              y++;break;
            }
          }
    ...
    if(c1){
      if(last==s1){
        s1+=s2[sz(s2)-1];debug(s1)
        if(palindrome(s1,sz(s1))){
          cout<<"YES"<<endl;c2++;return;
        }
      }
      else if(last==s2){
        s2+=s1[sz(s1)-1];debug(s2)
        if(palindrome(s2,sz(s2))){
          cout<<"YES"<<endl;c2++;return;
        }
      }
    }
