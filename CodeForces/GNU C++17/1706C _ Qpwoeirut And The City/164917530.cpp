...
void solve()
{
    ll m,n,i,j,k,x,y,z,sum=0,c1=0,c2=0,ans=0;
    cin>>n;
    vector<ll>a(n+5);
    for(int i=1;i<=n;i++)
    {
    ...
    vector<ll>oddSum(n+5,0),evenSum(n+5,0);
    ans=0;
    for(int i=2;i<=n;i+=2)
    {
      x=0,y=0;
    ...
    for(int i=n-1;i>=1;i-=2)
    {
      ...
      if(a[i]<=a[i+1])x=(a[i+1]-a[i])+1;
      if(a[i]<=a[i-1])y=(a[i-1]-a[i])+1;
      ans+=max(x,y);oddSum[i]=ans;  
    }
    bug2(oddSum,evenSum)
    if(n%2){
      cout<<evenSum[n-1]<<endl;
    }
    else{
      ans=INF;
      for(int i=1;i<=n;i+=2){
        x=0,y=0;
        x=evenSum[i-1]+oddSum[i+2];
        ans=min(ans,(x+y));debug(ans)
      }
      cout<<ans<<endl;
    }
