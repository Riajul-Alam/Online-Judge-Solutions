...
void solve()
{
    ...
    for(int i=1;i<=n;i++){
     if(i==1){
      ...
      pre[i]=pre[i-1]+(k-a[i]);
      if(i-1>=1){
       pre[i]+=max(a[i]-a[i-1]-1,0LL);
      }else{
       pre[i]+=(a[i]-1);
      }
     }
     else{
      pre[i]=pre[i-1]+max(a[i+1]-a[i]-1,0LL)+max(a[i]-a[i-1]-1,0LL);
     }
    }
    debug(pre)
    while(m--)
    {
     ...
     if(x==y){
      ...
      ans=(a[x]-1)+(k-a[y])+2*(max(a[y]-a[x]-1,0LL));
     }else{
      if(x==y){
       ans=k-1;
      }
