...
/*#define Max 100005
bool vis[Max];
vector<vector<ll>>g[M  ax];*/
ll mxAns=0;
pair<ll,ll>dfs(ll node,vector<vector<ll>>&a,vector<bool>&vis)
{
  vis[node]=1;
  pair<ll,ll>ans,total;ans={ll(0),ll(0)};
  ll white=0,black=0;
  if(s[node-1]=='B')black++;
  else white++;
  total={white,black};
  ...
  bug2(white,black)
  if(total.ff==total.ss && total.ff!=0){
   mxAns++;
  }
  return total;
}
