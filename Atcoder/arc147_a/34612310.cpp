...
void solve()
{
    ...
    {
      cin>>a[i].ff>>a[i].ss;
      mp[{a[i].ff,a[i].ss}]=i+1;
    }
    for(int i=0;i<n;i++){
    	string currentMin=a[i].ff;
    	ll currentMinIndex=i;
    	for(int j=i+1;j<n;j++){
    		string comp=a[j].ff;
    		if(comp==currentMin){
    		...
    		else if(comp<currentMin){
    			currentMin=comp;
    			currentMinIndex=j;
    		}
    	}
    	cout<<mp[{a[currentMinIndex].ff,a[currentMinIndex].ss}]<<endl;
    	swap(a[currentMinIndex],a[i]);
    }
}
