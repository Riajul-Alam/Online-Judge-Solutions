...
#define rf2(i,r,l) for(int i=r-1;i>=l;i--)
#define f3(i,l,r) for(int i=l;i<=r;i++)
#define rf3(i,r,l) for(int i=r;i>=l;i--)
...
/*    f0(n){
    ...
    ans=max(up[n-1],down[n-1]);
    debug(ans)
    ans=max(ans,max(up[0],down[0]));
    debug(ans)
    i=1,j=n-1;
    while(i<n && j>0)
    {
     bug2(i,j)
     ans=max(ans,(up[i]+down[j]));i++,j--;
    }
/*    i=1,j=n-1;
    while(i<n && j>0)
    {
     bug2(i,j)
     ans=max(ans,(down[i]+up[j]));
     i++,j--;
    }*/
   // debug(up)
    //debug(down)
   // debug(totalone)
    x=totalone+(n-ans)-ans;
    cout<<x<<endl;
