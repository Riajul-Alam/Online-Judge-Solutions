...
string s,tmp1;
ll dfs(ll node,vector<vector<ll>>&a,vector<bool>&vis)
{
  vis[node]=1;
  ll x=1;tmp1+=s[node-1];
...
void solve()
{
    ...
    for(ll i=1;i<=n;i++){
     if(!visit[i] && a[i-1]!=i){
      ...
      string tt=tmp1;debug(tt)
      c1=1;rotate(tt.begin(),tt.begin()+1,tt.end());
      while(tt!=tmp1){c1++;rotate(tt.begin(),tt.begin()+1,tt.end());}
      tmp.pb(c1);tmp1.clear();
     }
    }
    debug(tmp)
    if(sz(tmp)==0){
