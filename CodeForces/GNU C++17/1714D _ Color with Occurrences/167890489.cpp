...
void solve()
{
    ...
    map<string,ll>pos;
    f0(m){cin>>a[i];pos[a[i]]=i+1;tmp.pb({sz(a[i]),a[i]});y=sz(a[i]);mxsz=max(y,mxsz);}
    sort(rall(tmp));
    f0(m){
     a[i]=tmp[i].ss;
    }
 ...
 for(int i=0;i<n;i++)
 {
  ...
  bool flag=0;
  ll mxind=0;
  ll curid=0,startpos=i+1;
  pair<ll,ll>pr;pr={-1,-1};
  for(int j=0;j<m;j++)
  {
   //cout<<"YES"<<endl;
   curid=i+(sz(a[j])-1);debug(curid)
   if(curid>=n)curid=n-1;
   while(curid>=i && curid<n)
   {
    string t="";
    if(curid-(sz(a[j])-1)>=0)t=s.substr(curid-(sz(a[j])-1),sz(a[j]));
    debug(t)debug(a[j])
    if(t==a[j]){
     if(curid>=pr.ff){
      pr.ff=curid;
      pr.ss=pos[t];flag=1;
      startpos=curid-(sz(t)-1);
     }
    }
    curid--;
   }
