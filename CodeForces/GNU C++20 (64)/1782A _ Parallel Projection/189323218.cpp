...
void solve()
{
    ll n,m,w,k,d,x,h,y,z,sum=0,c1=INF,c2=INF,ans=INF;
    cin>>w>>d>>h;ll x1,y1,x2,y2;
    cin>>x1>>y1>>x2>>y2;
    ans=y1+h+y2+abs(x1-x2);
    ll ans1=INF,ans2=INF,ans3=INF,ans4=INF;
    ans1=h+abs(y1-d)+abs(d-y2)+abs(x1-x2);
    ans2=h+y1+y2+abs(x2-x1);
    ans3=h+abs(w-x1)+abs(w-x2)+abs(y1-y2);
    ans4=h+x1+x2+abs(y1-y2);
